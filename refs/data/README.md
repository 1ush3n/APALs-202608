...
`scripts/generate_initial_buckets.py`

### 11.2 功能

基于明确指定的 APAL 模板，通过拓扑安全的删除和边细分生成同规模变体。生成后检查紧前引用、DAG、技能覆盖和需求人数约束。

### 11.3 当前合成数据集

生成数据位于 `data/generated/initial_*/`，不纳入 Git。每个目录包含基准图副本、10 个变体和 `manifest.json`，禁止跨四种规模混合训练。
