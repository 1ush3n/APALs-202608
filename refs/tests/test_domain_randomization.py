...
from configs import configs as cfg, Config
...
def test_worker_pool_random_sampling():
    ...
    check(env.num_workers == cfg.n_w, f"randomized worker identities keep fixed count {cfg.n_w}")
    ...
    check(env.num_workers == cfg.n_w,
          f"eval worker count remains fixed at {cfg.n_w}")
...
def test_multi_dataset_switching():
    print("\n--- test_narrow_pool_switching ---")
    cfg.n_w = 40
    cfg.n_m = 5

    with tempfile.TemporaryDirectory() as temp_dir:
        pool_dir = Path(temp_dir)
        source = Path(ROOT_DIR) / "data" / "283.csv"
        shutil.copy2(source, pool_dir / "baseline_283.csv")
        shutil.copy2(source, pool_dir / "variant_01_283.csv")
        env = AirLineEnv_Graph(data_path_or_dir=pool_dir, seed=42)
        check(len(env.dataset_pool) == 2, f"Narrow pool has 2 entries (got {len(env.dataset_pool)})")

        n_tasks = env.num_tasks
        env.switch_dataset(1)
        state_0 = env.reset()
        check(state_0['task'].x.shape[0] == n_tasks,
              f"Same-scale switch preserves task count ({n_tasks})")

        env.switch_dataset(0)
        state_1 = env.reset()
        check(state_1['task'].x.shape[0] == n_tasks,
              f"Switch back restores baseline context ({n_tasks})")
