...
def test_vector_env_rebuild_and_switch() -> None:
    ...
    # 创建两个同规模图，验证窄池同步切图而不引入跨尺度轨迹。
    temp_pool = tempfile.TemporaryDirectory()
    datasets_dir = Path(temp_pool.name)
    shutil.copy2(PROJECT_ROOT / "data" / "283.csv", datasets_dir / "baseline_283.csv")
    shutil.copy2(PROJECT_ROOT / "data" / "283.csv", datasets_dir / "variant_01_283.csv")
    ...
    try:
        ...
        # 随机指定一个新索引进行切图
        num_datasets = vec_env.envs[0].dataset_count
        print(f"  当前图纸池包含图纸数: {num_datasets}")
        
        # 固定切到第二个同规模变体，确保真实覆盖切图路径。
