...
class RolloutHeartbeat:
    """训练采样阶段的轻量心跳，用于定位长时间无输出时卡住的阶段。"""

    def __init__(self, episode: int, num_envs: int, interval_sec: float = 30.0):
    ...
    def start(self) -> None:
        if self.interval_sec <= 0:
            return
        self._thread = threading.Thread(target=self._run, name=f"rollout-heartbeat-{self.episode}", daemon=True)
        self._thread.start()
    ...
    def update(
        ...
        if active is not None:
            self.state["active"] = int(active)
        if done is not None:
            self.state["done"] = int(done)
        if last_step_ms is not None:
            self.state["last_step_ms"] = float(last_step_ms)
        self.state["stage_since"] = now

    def _run(self) -> None:
        while not self._stop_event.wait(min(5.0, max(1.0, self.interval_sec / 3.0))):
            now = time.perf_counter()
            if now - float(self.state["last_print"]) < self.interval_sec:
                continue
            self.state["last_print"] = now
            stage_elapsed = now - float(self.state["stage_since"])
            print(
                f"[RolloutHeartbeat] ep={self.episode} stage={self.state['stage']} "
                f"step={self.state['step']} active={self.state['active']}/{self.num_envs} "
                f"done={self.state['done']}/{self.num_envs} "
                f"stage_elapsed={stage_elapsed:.1f}s last_step={self.state['last_step_ms']:.1f}ms",
                flush=True,
            )
...
def train(args):
    try:
        ...
        if start_method == "auto":
            start_method = "fork" if plat == "Linux" else "spawn"
        worker_threads = getattr(configs, 'vector_env_worker_threads', 'auto')
        print(f"VectorEnv worker_threads 配置: {worker_threads}")
        ...
        vec_env = VectorEnv(make_env, num_envs=num_envs, start_method=start_method, worker_threads=worker_threads)
        ...
        print(
            ...
            f"update_every_episodes={update_every_episodes}, "
        ...
        for ep in range(start_episode, configs.max_episodes + 1):
            heartbeat = RolloutHeartbeat(
                episode=ep,
                num_envs=num_envs,
                interval_sec=float(getattr(configs, 'rollout_heartbeat_interval_sec', 0.0)),
            )
            heartbeat.start()
            ...
            heartbeat.update("reset_all", step=0, active=0, done=0)
            states = vec_env.reset_all(randomize_duration=apply_noise, randomize_workers=apply_noise)
            dones = [False] * num_envs
            heartbeat.update("rollout_start", step=0, active=num_envs, done=0)
            ...
            for t in range(max_steps):
                ...
                heartbeat.update(
                    "get_masks_and_snapshots_all" if use_fast_path else "get_masks_all",
                    step=t,
                    active=sum(1 for d in dones if not d),
                    done=sum(1 for d in dones if d),
                    last_step_ms=last_rollout_step_ms,
                )
                ...
                heartbeat.update("deadlock_check", step=t, active=len(active_indices), done=num_envs - len(active_indices))
                ...
                heartbeat.update("select_actions_batch", step=t, active=len(active_indices), done=num_envs - len(active_indices))
                ...
                heartbeat.update("step_snapshot_all" if use_fast_path else "step_all", step=t, active=len(active_indices), done=num_envs - len(active_indices))
                if use_fast_path:
                ...
                if use_profiler:
                    ...
                    prof_step_count += 1
                last_rollout_step_ms = (time.perf_counter() - prof_step_t0) * 1000.0
            ...
            # Episode Summary
            heartbeat.update("episode_summary", step=t if 't' in locals() else 0, active=sum(1 for d in dones if not d), done=sum(1 for d in dones if d), last_step_ms=last_rollout_step_ms)
            ...
            if ep % update_every_episodes == 0:
                try:
                    heartbeat.update("ppo_update", step=max_steps, active=0, done=sum(1 for d in dones if d))
                    ...
                    is_cuda_oom = (
                        ...
                        "out of memory" in oom_msg and ("cuda" in oom_msg or "gpu" in oom_msg or "device" in oom_msg)
                    )
                    if is_cuda_oom:
                        ...
                        print(f"\n❌ [非OOM异常] agent.update() 发生非内存错误: {e.__class__.__name__}: {e}")
                        raise e
                finally:
                    memory.clear()
                    
                # 窄规模池轮换：仅在 update 完成且 memory 清空后同步切图。
                ...
                if current_update_count % getattr(configs, 'switch_dataset_every_updates', 1) == 0:
                    if env.dataset_count > 1:
                        ...
                        heartbeat.update("switch_dataset_all", step=max_steps, active=0, done=sum(1 for d in dones if d))
                        vec_env.switch_dataset_all(next_idx)
                        print(
                            f"      [Narrow Pool] dataset={next_idx + 1}/{env.dataset_count}, "
                            f"nodes={env.num_tasks}"
                        )
                
            # 定期评估与保存
              # [Validation Strategy]
            if ep % configs.eval_freq == 0:
                print(f"[Eval] start ep={ep}", flush=True)
                heartbeat.update("eval_start", step=max_steps, active=0, done=sum(1 for d in dones if d))
                if getattr(configs, "enable_reschedule_mode", False):
                    ...
                    res_metrics = {}
                heartbeat.update("eval_summary", step=max_steps, active=0, done=sum(1 for d in dones if d))
                print(f"[Eval] done ep={ep}", flush=True)
