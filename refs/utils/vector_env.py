...
def _sanitize_snapshot_tensors(snap: dict) -> None:
    """将 snapshot 中的 torch.Tensor 就地替换为 numpy，避免 IPC 共享内存泄漏。"""
    if 'base_worker_x' in snap:
        bw = snap['base_worker_x']
        if hasattr(bw, 'numpy'):
            snap['base_worker_x'] = bw.numpy()


def _worker(conn, make_env_fn: Callable[[int], Any], index: int, worker_threads: int):
    """
    运行在独立子进程中的环境步进循环。
    ...
    init_info = {
        ...
        'dataset_count': len(getattr(env, 'dataset_pool', [])),
        'active_dataset_idx': getattr(env, 'active_dataset_idx', 0),
        'worker_audit': {
            'pid': os.getpid(),
    ...
    while True:
        ...
        try:
            ...
            if cmd == 'step':
                ...
                masks = env.get_masks()
                masks_np = tuple(m.numpy() for m in masks)
                snap = env.get_state_snapshot()
                _sanitize_snapshot_tensors(snap)
                dynamic_info = {
                    'station_wall_clock': getattr(env, 'station_wall_clock', None),
                    'assigned_tasks': getattr(env, 'assigned_tasks', None),
                    'task_status': getattr(env, 'task_status', None),
                ...
                conn.send(("OK", (masks_np, snap, dynamic_info)))
                ...
                snap = env.get_state_snapshot()
                if 'base_worker_x' in snap and hasattr(snap['base_worker_x'], 'numpy'):
                    snap['base_worker_x'] = snap['base_worker_x'].numpy()
                conn.send(("OK", snap))
...
class EnvProxy:
    ...
    def update_static_properties(self, info: dict):
        ...
        if 'dataset_count' in info: self.dataset_count = int(info['dataset_count'])
        if 'active_dataset_idx' in info: self.active_dataset_idx = int(info['active_dataset_idx'])
    ...
    def get_rollout_state(self):
        ...
        status, val = self._conn.recv()
        if status == "OK":
            masks_np, snap, dynamic_info = val
            masks = tuple(torch.from_numpy(m) for m in masks_np)
            self.update_dynamic_properties(dynamic_info)
            return masks, snap
        else:
            raise val
...
class VectorEnv:
    ...
    def __init__(
        ...
        print(
            "[VectorEnv] "
            f"start_method={start_method or mp.get_start_method(allow_none=True) or 'default'} "
            f"num_envs={num_envs} os_cpu_count={os.cpu_count() or 1} "
            f"worker_threads={self.worker_threads} "
            f"parent_env_threads={{OMP:{os.environ.get('OMP_NUM_THREADS', '')}, "
        ...
        for i in range(num_envs):
            p = self._mp_ctx.Process(target=_worker, args=(self.child_conns[i], make_env_fn, i, self.worker_threads), daemon=True)
        ...
        for i in range(num_envs):
            ...
            if status == "INIT_OK":
                ...
                audit = info.get('worker_audit', {})
                self.worker_audits.append(audit)
                print(
                    "[VectorEnv worker] "
                    f"idx={i} pid={audit.get('pid')} "
                    f"torch_threads={audit.get('torch_num_threads')} "
                    f"OMP={audit.get('omp_num_threads')} "
                    f"MKL={audit.get('mkl_num_threads')} "
                    f"OPENBLAS={audit.get('openblas_num_threads')}"
                )
            else:
                self.close()
