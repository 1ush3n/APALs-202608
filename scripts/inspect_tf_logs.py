...
def parse_tf_logs(log_dir: Path | str, last_n: int = 10):
    ...
    print(f"已记录标量字段数: {len(tags)} | 分组: {list(groups.keys())}\n")
    ...
    for group_name, group_tags in groups.items():
        ...
        for tag in group_tags:
            ...
            if len(steps_vals) > 1:
                ...
                print(f"  • {tag:35s}: {latest_val:.4e} | 最近走势: [{sample_str}]{trend}")
            else:
                print(f"  • {tag:35s}: {latest_val:.4e} (step {steps_vals[0][0]})")
        print()
...
if __name__ == "__main__":
    ...
    parse_tf_logs(args.log_dir, args.last_n)
