...
EXPECTED_GROUPS = {
    "r3_temp000_seed42": (0.0, 42),
    **{f"r3_temp001_seed{seed}": (0.01, seed) for seed in range(42, 47)},
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="整理并审计 r3 六组重调度验证")
    ...
    file_manifest = [
        ...
        if path.is_file() and "eval_inconsistent" not in path.parts and path.name != "file_manifest.json"
    ] + integrity["checkpoint_files"]
    write_json(eval_root / "file_manifest.json", file_manifest)
    readme = f"""# r3 重调度六组验证结果

- 协议：r3 五技能；四个真实实例各评估 3 个固定场景（low_000、medium_000、high_000），工人数为 80/100/140/160。
- 验证组：temperature=0.0/seed42，以及 temperature=0.01/seed42–46，共 6 组。
- 原始行数：{len(frame)}（每实例 3×6=18 行）；complete_rate={integrity['complete_rate']:.4f}；eligible_rate={integrity['eligible_rate']:.4f}。
- 全部硬约束最大值：{max(integrity['hard_constraint_max'].values() or [0.0]):.4f}。
- `eval_inconsistent/` 中的旧验证结果仅作历史证据，不参与正式汇总。
- checkpoint 文件及 SHA-256 记录于 `integrity_check.json` 和 `file_manifest.json`。

正式文件：`summary.csv/json`、`reschedule_eval_by_level.csv`、`reschedule_eval_by_instance_level.csv`、`raw_sixrun_rows.csv`、`integrity_check.json`。
"""
