...
echo "1. initial_operation_only_strict (60 ep, batch=32, envs=2, BF16)"
echo "2. initial_operation_station_strict (60 ep, batch=32, envs=2, BF16)"
echo "3. initial_homogeneous_graphsage_strict (60 ep, batch=32, envs=2, BF16)"
...
${PYTHON} train.py   experiment=initial_operation_only_strict   train.batch_size=32   train.num_envs=2   hardware.num_envs=2   hardware.worker_pointer_v2_fast_default_num_envs=2   train.max_episodes=60   seed=42   > logs/train_initial_operation_only_strict.log 2>&1 &
PID_OP_ONLY=$!

echo ">>> Launching [2/3] Initial Operation+Station Strict in background..."
${PYTHON} train.py   experiment=initial_operation_station_strict   train.batch_size=32   train.num_envs=2   hardware.num_envs=2   hardware.worker_pointer_v2_fast_default_num_envs=2   train.max_episodes=60   seed=42   > logs/train_initial_operation_station_strict.log 2>&1 &
PID_OP_STATION=$!

echo ">>> Launching [3/3] Initial Homogeneous GraphSAGE Strict in background..."
${PYTHON} train.py   experiment=initial_homogeneous_graphsage_strict   train.batch_size=32   train.num_envs=2   hardware.num_envs=2   hardware.worker_pointer_v2_fast_default_num_envs=2   train.max_episodes=60   seed=42   > logs/train_initial_homogeneous_graphsage_strict.log 2>&1 &
PID_GRAPHSAGE=$!
